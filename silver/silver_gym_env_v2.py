...
EVENT_FLAGS_LENGHT = (EVENT_FLAGS_END - EVENT_FLAGS_START) * 8
#missing bits since silver has less flags in total
EVENT_FLAGS_MISSING = 448

#MAP
CURRENT_MAP_BANK_ADDR = 0xDA00
CURRENT_MAP_NUMBER_ADDR = 0xDA01
CURRENT_PLAYER_X_POS_ADDR = 0xDA03
CURRENT_PLAYER_Y_POS_ADDR = 0xDA02

#BAGES
JOHTO_BADGE_FLAGS_ADDR = 0xD57C
KANTO_BADGE_FLAGS_ADDR = 0xD57D

#POKEMON
PARTY_COUNT_ADDR = 0xDA22
PARTY_POKEMON_ADDRS = [0xDA23, 0xDA24, 0xDA25, 0xDA26, 0xDA27, 0xDA28]
POKEMON_LEVELS_ADDRS = [0xDA49, 0xDA79, 0xDAA9, 0xDAD9, 0xDB09, 0xDB39]
POKEMON_HP_CURR_ADDRS = [0xDA4C, 0xDA7C, 0xDAAC, 0xDADC, 0xDB0C, 0xDB3C]
POKEMON_HP_MAX_ADDRS = [0xDA4E, 0xDA7E, 0xDAAE, 0xDADE, 0xDB0E, 0xDB3E]

#BATTLE
TYPE_OF_BATTLE_ADDR = 0xD116

class SilverGymEnv(Env):
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "events": spaces.MultiBinary(EVENT_FLAGS_LENGHT + EVENT_FLAGS_MISSING),
    ...
    def _get_obs(self):
        ...
        level_sum = 0.02 * sum([
            self.read_m(a) for a in POKEMON_LEVELS_ADDRS
        ])
        ...
        observation = {
            ...
            "badges": np.array([int(bit) for bit in f"{self.read_m(JOHTO_BADGE_FLAGS_ADDR):08b}"], dtype=np.int8),
    ...
    def step(self, action):
        ...
        self.party_size = self.read_m(PARTY_COUNT_ADDR)
        ...
        self.agent_stats.append(
            {
                ...
                "last_action": action,
                "pcount": self.read_m(PARTY_COUNT_ADDR),
                "levels": levels,
    ...
    def get_game_coords(self):
        return (self.read_m(CURRENT_PLAYER_X_POS_ADDR), self.read_m(CURRENT_PLAYER_Y_POS_ADDR), self.read_m(CURRENT_MAP_NUMBER_ADDR), self.read_m(CURRENT_MAP_BANK_ADDR))

    def update_seen_coords(self):
        # if not in battle
        if self.read_m(TYPE_OF_BATTLE_ADDR) == 0:
            x_pos, y_pos, map_n, map_bank = self.get_game_coords()
            coord_string = f"x:{x_pos} y:{y_pos} m:{map_n} b:{map_bank}"
    ...
    def get_badges(self):
        johto = self.bit_count(self.read_m(JOHTO_BADGE_FLAGS_ADDR))
        kanto = self.bit_count(self.read_m(KANTO_BADGE_FLAGS_ADDR))
        return johto + kanto

    def read_party(self):
        return [
            self.read_m(addr)
            for addr in PARTY_POKEMON_ADDRS
        ]
    ...
    def update_heal_reward(self):
        ...
        if cur_health > self.last_health and self.read_m(PARTY_COUNT_ADDR) == self.party_size:
    ...
    def read_hp_fraction(self):
        hp_sum = sum([
            self.read_hp(add)
            for add in POKEMON_HP_CURR_ADDRS
        ])
        max_hp_sum = sum([
            self.read_hp(add)
            for add in POKEMON_HP_MAX_ADDRS
        ])
    ...
    def update_map_progress(self):
        map_idx = self.read_m(CURRENT_MAP_NUMBER_ADDR)
