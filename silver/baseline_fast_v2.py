...
from silver_gym_env_v2 import SilverGymEnv
...
if __name__ == "__main__":
    ...
    env_config = {
                ...
                'action_freq': 24, 'init_state': '../silver_init.state', 'max_steps': ep_length, 
                'print_rewards': True, 'save_video': False, 'fast_video': True, 'session_path': sess_path,
                'gb_path': '../PokemonSilver.gbc', 'debug': False, 'reward_scale': 0.5, 'explore_weight': 0.25
            }
    ...
    if exists(file_name + ".zip"):
        print("\nloading checkpoint")
        model = PPO.load(file_name, env=env, learning_rate=3e-5)
        ...
        model = PPO(
            ...
            env,
            verbose=1,
            learning_rate=3e-5,      # slower updates for fine tuning
            n_steps=train_steps_batch,            # smaller rollouts -> faster adaptation
            batch_size=512,
            n_epochs=8,              # more learning per batch
            gamma=0.997,
            ent_coef=0.02,           # more exploration
            target_kl=0.02,          # prevent destructive updates
